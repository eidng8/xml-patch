...
describe('Patch <add>', () => {
  ...
  it('appends new element', () => {
    ...
    expect(
      new Patch()
        .load('<diff><add sel="/*[local-name()=\'a\']"><c>w</c></add></diff>')
        .patch('<a/>'),
  ...
  it('allows adding empty attribute value', () => {
    ...
    expect(
      new Patch()
        .load('<diff><add sel="a/b" type="@c"/></diff>')
        .patch('<a>x<b>y</b>z</a>')
        .toString({ minify: true, preserveComments: true }),
    ).toBe('<a>x<b c="">y</b>z</a>');
    expect(
      new Patch()
        .load('<diff><add sel="a/b" type="attribute::c"/></diff>')
        .patch('<a>x<b>y</b>z</a>'),
  ...
  it('inserts after target comment', () => {
    ...
    expect(
      new Patch()
        .load('<diff><add sel="a" pos="after"><!-- a comment --></add></diff>')
        .patch('<a>x<b>y</b>z</a>'),
  ...
  it('throws if add attribute with non-text node', () => {
    ...
    expect(() =>
      new Patch()
        .load('<diff><add sel="a/b" type="@c"><a/></add></diff>')
        .patch('<a>x<b>y</b>z</a>'),
    ).toThrow(InvalidNodeTypes);
    expect(() =>
      new Patch()
        .load('<diff><add sel="a/b" type="attribute::c"><a/></add></diff>')
        .patch('<a>x<b>y</b>z</a>'),
    ).toThrow(InvalidNodeTypes);
  });
  ...
  it('throws if adding empty namespace', () => {
    ...
    expect(() =>
      new Patch()
        .load('<diff><add sel="a/b" type="namespace::c"/></diff>')
        .patch('<a>x<b>y</b>z</a>'),
    ).toThrow(InvalidPatchDirective);
  });
  ...
  it('throws if types is invalid', () => {
    ...
    expect(() =>
      new Patch()
        .load('<diff><add sel="a/b" type="wrong"/></diff>')
        .patch('<a>x<b>y</b>z</a>'),
